<template>
  <div class="flex flex-col w-full bg-white dark:bg-gray-950  lg:p-5 md:p-10 p-5">
    <div
    ...
    <div class="flex flex-col gap-y-2 rounded-md">
      <div class="border-b border-gray-100 dark:border-slate-800 my-4"></div>
      <!-- toggle contact btn -->

      <h2 class="text-lg dark:text-white">{{ merchant?.name }}</h2>
      <div class="flex items-center justify-between">
        <div class="d-flex gap-x-3 items-center">
          <Icon class=" text-teal-950 dark:text-white" name="material-symbols:call-outline" />
    ...
    <div class="flex flex-col">
      <div class="border-b border-gray-100 dark:border-slate-800 m my-4"></div>
      ...
      <div class="flex justify-between items-center mb-2">
        ...
        <div class="flex justify-center">
          <button
            ...
            class="flex gap-x-2 items-center border border-gray-300 dark:border-white w-fit px-2 py-1 rounded-full">
            <Icon
            ...
            <h2 v-if="showInvoiceItems" class="text-xs dark:text-white">Close invoice list</h2>
      ...
      <div class="flex flex-col my-4 gap-y-2">
        <h2 class="dark:text-white">Monthly Invoice for Coporate Car Wash</h2>
      ...
      <div
        ...
        :key="index"
        class="flex items-center gap-x-2 my-4">
        <div class="h-10 w-10 rounded-md bg-gray-100 dark:bg-gray-900 flex justify-center items-center">
        ...
        <div class="flex flex-col items-start h-full flex-1">
          ...
          <div class="flex gap-x-2">
            <p class="text-sm dark:text-white">{{ invoice?.currency }} {{ item.unit_price }}</p>
            <p class="text-sm dark:text-white">Qty : {{ item.quantity }}</p>
          </div>
        </div>
        <h2 class="text-base dark:text-white">{{ invoice?.currency }} {{ item.subtotal }}</h2>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-y-2">
      ...
      <div class="flex justify-between">
        ...
        <h2 v-if="invoice?.discount_type === null" class="text-sm dark:text-white">No discount</h2>
      ...
      <div class="flex justify-between">
        ...
        <h2 class="text-sm dark:text-white">{{ invoice?.currency }} {{ invoice?.discount_amount }}</h2>
      </div>
      ...
      <!-- tax -->
      <div class="flex justify-center items-center my-2 gap-x-3">
        <div class="border-b w-5 border-gray-200 dark:border-white m"></div>
      ...
      <div class="flex justify-between">
        ...
        <h2 class="text-sm dark:text-white">{{ invoice?.currency }} {{ invoice?.tax_amount }}</h2>
      </div>
      ...
      <div class="flex justify-between">
        ...
        <h2 class="text-lg dark:text-white">{{ invoice?.currency }} {{ invoice?.total }}</h2>
      </div>
    </div>
  </div>
</template>
