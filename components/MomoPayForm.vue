  ...
  <div>
    ...
    <MazStepper color="warning" auto-validate-steps :model-value="step">
      ...
      <template #content-2="{ nextStep, previousStep }">
        ...
        <MazInputCode color="warning" size="xs" :codeLength="6" v-model="ruleForm.otp" @completed="handlePayment"
          class="flex flex-wrap justify-start mt-10" />
        <div class="flex justify-between mt-5">
          <MazBtn color="transparent" size="sm" @click="previous()" class="w-1/2">Go back</MazBtn>
          <MazBtn color="warning" size="sm" @click="initiateOTPRequest" :loading="isSendOTPLoading" class="w-1/2">Resend
            code</MazBtn>
...
// use Momo payment
const { isPaymentFailed, isPaymentLoading, isPaymentSuccessful, isPaymentPromptInitiated, pay, statusText } = useMomoPayment()
